// * elemtento del pop-up
    // var pop_up = document.getElementById("pop-up");

// * elementos de la grafica de pastel
    // const $grafica = document.getElementById("myChart");

// * secciones
    const section_registro = document.getElementById("section_registro");
    // const section_grafica = document.getElementById("section_grafica");
    // const section_archivo = document.getElementById("section_archivo");
...
// * EVENTOS - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    // * EVENTOS DEL POP-UP PARA OCULTARLO, EN EL CASO DE QUE NO SE VEA EL BOTON
...
// * FUNCIONES  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    // * FUNCION PARA CREAR LA TABLA
        ...
        //     const etiquetas = ["En proceso", "Cancelada", "Terminada"];
        //     const datos = {
        //         label: "Desglose",
        //         data: [30, 60, 10],
        //         backgroundColor: [
        //             'rgba(240, 8, 8, 0.568)',
        //             'rgba(13, 17, 231, 0.959)',
        //             'rgba(90, 223, 13, 0.781)'
        //         ],
        //         borderColor:[
        //             'rgba(240, 8, 8, 0.568)',
        //             'rgba(13, 17, 231, 0.959)',
                ...
                // * Verificamos que la peticion sea exitosa
                    request.onreadystatechange = function(){
                        if (request.readyState == 4 && request.status == 200){
                            ...
                            error_box.classList.add('error-box--hide');
                            tabla2.classList.remove('tabla--hide');
                        }
                        else {
    ...
    // * FUNCION PARA PROCESAR Y MOSTRAR DATOS (SOLO LOS PRIMEROS 25 DE LA CONSULTA)
        function showData(datos){
            ...
            // PREGUNTO SI EXISTE PARA EVITAR GENERAR DUPLICADOS
                const qDivContendorPadre = document.getElementById("tablaContendorRegistros");
                
                if (document.body.contains(qDivContendorPadre)){
                    qDivContendorPadre.remove();
                    setTimeout(2000);
                }

            // CREO LA TABLA Y ASIGNO EL CONTENEDOR A UNA VARIBALE
                createTable();
                const divContendorPadre = document.getElementById("tablaContendorRegistros");
                setTimeout(2000);
            ...
            // CICLO INICAL QUE MUESTRA LOS PRIMEROS 25 REGISTROS
                for (let i = 0; i <= 24; i++) {
                    ...
                    // CICLO QUE PONE Y LEE LOS VALORES DE 0 - 8 PARA LA PRIMERA TABLA
                    for (let index = 0; index <= 8; index++) {
                        ...
                        divContendorPadre.appendChild(div);
                        div.appendChild(p);
    ...
    // * FUNCION PARA MOSTRAR LOS SIGUIENTES 25
        function showNextData(inicio, tope, pagina, datosNextPreviuos){
            ...
            // PRIMERO SE ELIMINAN LOS REGISTROS PREVIOS
                const nuevoDivContendorPadre = document.getElementById("tablaContendorRegistros");
            ...
            // PONEMOS LOS NUEVOS VALORES
                for (inicio; inicio <= tope; inicio++) {
                    ...
                    for (let index = 0; index <= 8; index++) {
                        ...
                        nuevoDivContendorPadre.appendChild(div);
                        div.appendChild(p);
    ...
    // * CREAR TABLA DE REGISTROS
        function createTable(){
            ...
            // * CREO EL CONTENDOR DE LOS REGISTROS DONDE IRA TODA LA INFORMACION DE LA GRANDE
                const divContendorRegistros = document.createElement('DIV');
                divContendorRegistros.setAttribute("id", "tablaContendorRegistros");
                divContendorRegistros.classList.add("tablaResumen");
                tabla2.appendChild(divContendorRegistros);

                const divContendorPadre = document.getElementById("tablaContendorRegistros");
            
            // * CREO LOS TITULOS
                const nombresTitulos = ['Sucursal', 'Id Bitacora', 'Fecha Salida', 'Fecha Factura', 'Factura', 'KMS Finales', 'KMS Facturados', 'N. Corte', 'Fin de Corte']

                for (let titulo = 0; titulo < nombresTitulos.length; titulo++) {
                    
                    const divTitulo = document.createElement("DIV");
                    divTitulo.classList.add("tabla__titulo");
                    const pContenido = document.createElement("P");
                    pContenido.textContent = nombresTitulos[titulo];

                    divContendorPadre.appendChild(divTitulo);
                    divTitulo.appendChild(pContenido);
                }

            // * CREO QUE LAS SEPARACION
                const separacion = document.createElement("DIV");
                separacion.classList.add("tablaResumen__separacion");
                divContendorPadre.appendChild(separacion);
        }